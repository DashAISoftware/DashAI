...
export const getComponents = async ({
  ...
  if (selectTypes.length > 0) {
    const selectTypesQuery = `select_types=${selectTypes.join('&select_types=')}`;
    queryArray.push(selectTypesQuery); 
  }

  if (ignoreTypes.length > 0) {
    const ignoreTypesQuery = `ignore_types=${ignoreTypes.join('&ignore_types=')}`;
  ...
  const response = await api.get<Array<ITask | IDataloader | object>>(`/v1/component/?${fullQuery}`);
