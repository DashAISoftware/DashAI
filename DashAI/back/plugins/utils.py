...
def _get_plugin_by_name_from_pypi(plugin_name: str) -> dict:
    ...
    response: requests.Response = requests.get(
        f"https://test.pypi.org/pypi/{plugin_name}/json"
    )
...
def install_plugin_from_pypi(pypi_plugin_name: str):
    ...
    """
    pypi_plugin_name = pypi_plugin_name.replace("-", "_")
    testpypi = "https://test.pypi.org/simple/"  # TODO: Use this only en dev mode
    flag = "-i"  # TODO: Use this only en dev mode
    res = subprocess.run(
        ["pip", "install", flag, testpypi, pypi_plugin_name],
