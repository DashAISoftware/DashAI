    ...
    def evaluate(self, model, input_dataset, output_dataset, metric, **kwargs):
        ...
        **kwargs
        ...
        for i in range(len(input_dataset) - 1):
            ...
            #
            # ``compute_metrics`` returns None when there was nothing to score
            # at all -- no metrics configured, or a partition with no rows --
            # which is not the same statement as "scored, and the result was
            # empty". A fold with nothing to score cannot contribute a number
            # to the mean below, and averaging over the folds that did have
            # data would report a score for a search the user cannot reproduce.
            train_scores = (
            ...
            if validation_scores is None:
                raise ValueError(
                    f"Fold {i} has no validation data to score, so the "
                    "cross-validation objective cannot be computed. Check the "
                    "number of folds against the size of the dataset."
                )
