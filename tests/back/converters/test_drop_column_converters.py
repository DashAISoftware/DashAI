...
from DashAI.back.converters.column_dropper import ColumnDropper
...
def test_remove_input_column(
    iris_dataset: DatasetDict, iris_dataset_petal_width_dropped: DatasetDict
):
    dropper = ColumnDropper()
    dataset_obtained = dropper.transform(iris_dataset, [3])
...
def test_remove_2_input_columns(
    iris_dataset: DatasetDict, iris_dataset_petal_cols_dropped: DatasetDict
):
    dropper = ColumnDropper()
    dataset_obtained = dropper.transform(iris_dataset, [2, 3])
