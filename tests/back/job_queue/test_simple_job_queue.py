...
def test_empty_queue(job_queue: BaseJobQueue):
    ...
    job = Job(func=lambda x: x, type=JobType.runner, run_id=0)
    job_queue.put(job)
...
def test_queue_to_list(job_queue: BaseJobQueue):
    ...
    job_1 = Job(func=lambda x: x, type=JobType.runner, run_id=0)
    job_1_id = job_queue.put(job_1)
    jobs = job_queue.to_list()
    assert len(jobs) == 1
    assert jobs[0].id == job_1_id

    job_2 = Job(func=lambda x: x, type=JobType.runner, run_id=0)
...
def test_enqueue_jobs(job_queue: BaseJobQueue):
    ...
    job_3 = Job(func=lambda x: x, type=JobType.runner, run_id=0)
