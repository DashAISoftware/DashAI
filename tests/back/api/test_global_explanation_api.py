...
def test_create_global_explanation(client: TestClient, run_id_1: int, run_id_2: int):
    ...
    with session() as db:
        ...
        explanation_1 = db.get(GlobalExplanation, 1)
        explanation_2 = db.get(GlobalExplanation, 2)
        explanation_3 = db.get(GlobalExplanation, 3)
        db.delete(explanation_1)
        db.delete(explanation_2)
        db.delete(explanation_3)
        db.commit()
        db.close()


def test_get_global_explanation_by_id(client: TestClient, run_id_1: int):
    container = client.app.container
    session = container.db.provided().session
    ...
    with session() as db:
        ...
        explanation = db.get(GlobalExplanation, 1)
